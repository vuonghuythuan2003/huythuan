...
$kiemTraQuery = "SELECT id_name FROM test1 WHERE id_name = '$userId'";
...
if ($kiemTraResult->num_rows == 0) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sqlTest2 = "INSERT INTO test1 (" . implode(", ", array_keys($valuesTest2)) . ") VALUES ('" . implode("', '", $valuesTest2) . "')";

        if ($conn->query($sqlTest2) !== TRUE) {
            echo "Lỗi khi chèn dữ liệu vào CSDL (test2): " . $conn->error;
        } else {
            // Cập nhật dữ liệu trong bảng user_tests
            $capNhatQuery = "UPDATE user_tests 
                            SET test1 = '$totalScore',
                            time_start = NOW(),
                                time_taken = NOW(), 
            ...
            if ($conn->query($capNhatQuery) !== TRUE) {
                ...
                echo '<a href="test1_ls.php"> xem lại lịch sự</a>';
                echo '</div>';
            }
        }
    }
